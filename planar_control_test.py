...
# this file is for the control test, this is not a formal file
...
# # test one step optimization problem
...
# Rz = np.eye(2) * 0.1
# Ru = np.eye(2)
# z = cp.Variable(2) # latent representation
# cost = cp.quad_form(z, Rz) 
# constr = [z_next == A_t.bmm(z.unsqueeze(-1)).squeeze(-1) + B_t.bmm(u_t.unsqueeze(-1)).squeeze(-1) + o_t ]
# problem = cp.Problem(cp.Minimize(cost), constr)
# problem.solve()
# error:  A_t.bmm(z_bar_t.unsqueeze(-1)).squeeze(-1) + B_t.bmm(u_t.unsqueeze(-1)).squeeze(-1) + o_t
